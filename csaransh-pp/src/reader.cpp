...
std::array<std::string, 2> csaransh::separateFileAndExt(std::string path) {
  std::size_t extPos = path.find_last_of(".");
  std::string fname;
  if (extPos != std::string::npos) fname = path.substr(0, extPos);
...
std::pair<std::string, std::string> csaransh::getInfileFromXyzfile(std::string xyzfile) {
  ...
  return std::make_pair(dirFile[0] + "/" + fnameExt[0] + ".in", fnameExt[0]);
}

csaransh::readStatus processFileParcasLammps(std::string xyzfile, std::string infile, std::string tag, std::ofstream &outfile, int id, csaransh::SimulationCode sc) {
  csaransh::Info info; bool isInfo;
  std::tie(info, isInfo) = (sc == csaransh::SimulationCode::parcas) ? csaransh::extractInfoParcas(infile) : csaransh::extractInfoLammps(infile);
  info.infile = tag;
  ...
  csaransh::printJson(outfile, tag, info, id, nDefects, nClusters, maxClusterSizeI, maxClusterSizeV, inClusterFractionI,
                      inClusterFractionV, defects, distances, angles, clusterIdMap, clusterIVMap, clusterFeats);
  return csaransh::readStatus::success;
}

csaransh::readStatus processFileDisplacedAtoms(std::string xyzfile, std::string infile, std::string tag, std::ofstream &outfile, int id, csaransh::SimulationCode sc) {
  csaransh::Info info; bool isInfo;
  std::tie(info, isInfo) = csaransh::extractInfoLammps(infile);
  info.infile = tag;
  if (!isInfo) return csaransh::readStatus::fail;
  info.simulationCode = sc;
  auto defects = csaransh::groupDefects(csaransh::displaced2defects(xyzfile, info), info.latticeConst);
  auto clusterSizeMap = csaransh::clusterSizes(defects);
  csaransh::ignoreSmallClusters(defects, clusterSizeMap, 2, 4);
  auto clusterIdMap = csaransh::clusterMapping(defects);
  auto clusterIVMap = csaransh::clusterIVType(clusterIdMap, clusterSizeMap);
...
auto getSimulationCode(std::string fname) {
  std::ifstream infile(fname);
  if (!infile.bad() && infile.is_open()) {
    std::string line;
    int lineNo = 0;
    ...
    std::vector<csaransh::SimulationCode> codes {
      csaransh::SimulationCode::parcas,
      csaransh::SimulationCode::lammps,
      csaransh::SimulationCode::lammpsDisplacedCompute
    };
    while (std::getline(infile, line) && lineNo++ < 10) {
...
csaransh::readStatus csaransh::processFile(std::string xyzfile, std::ofstream &outfile, int id) {
  ...
  if (sc == csaransh::SimulationCode::parcas || sc == csaransh::SimulationCode::lammps) {
