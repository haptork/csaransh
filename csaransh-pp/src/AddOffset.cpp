...
bool AddOffset::_isUnitcell(double x, double y, double z, double l,
 ...
 * TODO: first four, why do we get more atoms in certain locations when counting?
 * */
void AddOffset::_fccUnitcell() {
  /*
  for (int i = -1; i < 3; i++) {
    for (int j = -1; j < 3; j++) {
      for (int k = -1; k < 3; k++) {
  ...
  _sites.emplace_back(std::array<double, 3>{{0.0, 0.0, 0.0}});
  _sites.emplace_back(std::array<double, 3>{{0.5, 0.5, 0.0}});
  _sites.emplace_back(std::array<double, 3>{{0.5, 0.0, 0.5}});
  _sites.emplace_back(std::array<double, 3>{{0.0, 0.5, 0.5}});
  for(auto& it : _sites) {
    auto i = 0;
    for (auto& jt : it) {
      jt += origin[i];
...
double AddOffset::_calcDistMirror(std::array<long double, 3> a, std::array<long double, 3> b,
...
AddOffset::AddOffset(double latConst, std::string lattice,
                     std::array<double, 3> origin) {
...
AddOffset::operator()(const std::array<double, 3> &c) {
