...
const calcDefectSizeDistribution = (row) => {
  ...
  function addToDataSet(newDataSets, d, type, len) {
    ...
    for (const i in d[0]) {
      ...
      if (x) {
        ...
        //console.log("z: " + d[0][i] + y);
        newDataSets.push({
          label: label,//this.cookLabel(d[0][i], type, y),
          //backgroundColor: getColor(d[0][i] - 1),
          stack: stack,
  ...
  function addedDataSets(curDef, row) {
    ...
    if (!curDef.datasets || curDef.datasets.length == 0) {
      for (const i in d[0][0]) {
        newDataSets.push({
            ...
            //backgroundColor: getColor(d[0][0][i] - 1/*newDataSets.length*/),
    ...
    newDataSets.sort((a, b) => {
      /*
      let res = b.stack - a.stack;
      if (res == 0) res = a.clusterSize - b.clusterSize;
      return res;
      */
  ...
  export function lookDefectsSizeDistrib(curDe, row, limitSize) {
    let curDef = {labels:curDe.labels.slice(), datasets:curDe.datasets.slice()};
    const cookDataLabel = name => ["int | vac", name];
    for (const x of curDef.labels) {
      if (x[1] == row.name) return curDef;
    }
    if (curDef.labels.length === limitSize) {
      curDef = removeDefectsSizeDistrib(curDe, {name:curDef.labels[1][1]});
    }
    return {
